...
bs_upload(char *buf, int len)
{
    ...
    rc = cbor_parser_init(buf, len, 0, &parser, &map);
    if (!rc) {
        if (cbor_value_map_find_value(&map, "off", &value) == 0) {
            rc = cbor_value_get_uint64(&value, &off);
            if (rc) {
                rc = 10;
                goto out;
            }
        ...
        // Len is only provided when off=0
        if (cbor_value_map_find_value(&map, "len", &value) == 0) {
            rc = cbor_value_get_uint64(&value, &data_len);
            if (rc) {
                rc = 11;
                goto out;
            }

            // Slot is encoded in top byte to avoid need for another map entry
            slot = (data_len & 0x0F000000) >> 24;
            // Can only be 0 or 1
            if ((slot < 0) || (slot > 1)) {
                slot = 0;
            }
        ...
        if (cbor_value_map_find_value(&map, "data", &value) == 0) {
            img_blen = sizeof(img_data);
            rc = cbor_value_copy_byte_string(&value, img_data, &img_blen, NULL);
            if (rc) {
                rc = 12;
                goto out;
            }
...
boot_serial_start(int max_input)
{
...
#ifdef __ZEPHYR__
#if 1
    long long unsigned int idle_count = 0; // IRIS CHANGE
#endif
...
#if 1
    // IRIS Change - let host know we are ready!
    console_write("ready\n", 6);
#endif
    ...
    while (1) {
        ...
        if (rc <= 0 && !full_line) {
#if 1
            // IRIS CHANGE - exit serial upgrade mode if no activity
            //  This gives us a few seconds to start and upgrade after reset
            idle_count++;
